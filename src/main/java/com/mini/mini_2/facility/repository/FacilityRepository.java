...
public interface FacilityRepository extends JpaRepository<FacilityEntity, Integer> {
    ...
    // 주어진 시설들을 갖춘 휴게소 ID 반환(ex 약국이 있는 휴게소만 출력) 
    @Query("SELECT f.restArea.restAreaId " + 
           "FROM   FacilityEntity f " + 
           "WHERE  f.name IN :types " +
           "GROUP BY f.restArea.restAreaId " +
           "HAVING COUNT(DISTINCT f.name) = :#{#types.size()}")
    List<Integer> findRestAreaIdsByTypes(@Param("types") List<String> types); 
